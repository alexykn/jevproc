...
for process, label, p, confidence, risk in itertools.product(
    processes, ["legitimate", "suspicious", "unknown"], [0.4, 0.85, 0.96], [0.1, 0.7, 0.9], [0.01, 0.7]
):
    probabilities = {"legitimate": 0.01, "suspicious": risk, "unknown": 0.01}
...
score_rule = Rule.model_validate({
    "id": "LOCALS",
    "title": "test",
    "message": "review",
    "question": {
...
for process, value, confidence in itertools.product(
    processes, [0.0, 0.5, 1.49, 1.5, 1.99, 2.0, 2.5, 3.0], [0.1, 0.7, 0.99]
):
    answer = ScoreAnswer(
        type="score", score=value, confidence=confidence, probabilities={str(i): 0.25 for i in range(4)}
    )
...
async def handler(request):
    ...
    score = {"benign": 0.07, "ambiguous": 0.09, "suspicious": 0.2, "unknown": 0.04}[case.label]
...
outputs = []
with (
    patch.object(corpus_cli, "JevClient", mock_client),
    patch.dict(os.environ, {"TYPESAFE_API_KEY": "synthetic-contract-key"}),
):
    ...
    json.dumps(
        {
            ...
            "decision_digest": digest(results),
            "redactions": len(redacted),
            "redaction_digest": digest(redacted),
            "sanitized_snapshot_digest": digest(sanitize_snapshot(snapshot, True).model_dump()),
            "cli_variants": len(outputs),
            "cli_digest": digest(outputs),
        },
        sort_keys=True,
        indent=2,
    )
