...
def parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="jevproc",
        description="Read-only process triage with Jev. Warnings and uncertain warnings are shown by default.",
    )
    ...
    mode.add_argument(
        "--demo", action="store_true", help="run clearly labeled synthetic fixtures; no API key or live collection"
    )
    target = p.add_mutually_exclusive_group()
    target.add_argument("--pid", type=positive_int, action="append", help="inspect only a PID (repeatable)")
    target.add_argument(
        "--family", type=positive_int, metavar="PID", help="inspect a PID and its descendant process family"
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        help="collect and submit redacted command arguments (enabled by default)",
    )
    p.add_argument(
        "--no-command-line", action="store_true", help="omit command arguments from collection and Jev requests"
    )
    p.add_argument(
        "--no-connections", action="store_true", help="do not collect sockets (does not disable the Jev API)"
    )
    ...
    p.add_argument(
        "--config", type=Path, help="explicit YAML overrides; no implicit current-directory config discovery"
    )
    p.add_argument("--print-default-config", action="store_true", help="print the packaged YAML and exit")
    p.add_argument(
        "--ignore", action="append", default=[], metavar="RULE_OR_SET", help="disable a rule/ruleset (repeatable)"
    )
    ...
    p.add_argument(
        "--collection-workers", type=positive_int, help="maximum concurrent local evidence-collection workers"
    )
    p.add_argument(
        "--max-requests", type=positive_int, help="hard attempt budget including retries across watch cycles"
    )
    ...
    p.add_argument(
        ...
        type=interval,
        metavar="SECONDS",
        help="repeat snapshots after this delay; append reports without clearing the screen",
    )
    p.add_argument(
        "--format", choices=["text", "json", "jsonl"], default="text", help="JSON formats always include all processes"
    )
    p.add_argument("--width", type=positive_int, help="terminal width (minimum 20 columns)")
    p.add_argument("--color", choices=["auto", "always", "never"], default="auto")
    p.add_argument(
        "--fail-on",
        choices=["warning", "any", "none"],
        default="warning",
