...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--pid", type=positive_int, action="append", help="inspect a PID (repeatable; ancestry is limited to selected processes)")
    ...
    p.add_argument("--include-command-line", action="store_true", help="collect and submit redacted command arguments; redaction is best effort")
    p.add_argument("--no-connections", action="store_true", help="do not collect sockets (does not disable the Jev API)")
    p.add_argument("--hashes", action="store_true", help="hash bounded regular executable files; binary content is never submitted")
    p.add_argument("--signatures", action="store_true", help="verify on-disk macOS signatures using codesign; this is not a safety guarantee")
    ...
    p.add_argument("--concurrency", type=positive_int, help="bounded API worker count")
