...
@pytest.mark.parametrize(
    ...
    [
        ...
        (0.10, "uncertain_warning"),
        (0.119, "uncertain_warning"),
        (0.12, "warning"),
        (0.40, "warning"),
        (0.99, "warning"),
    ],
...
def test_high_probability_with_partial_evidence_is_only_tentative(config, snapshot):
    ...
    result = judge(config.active_rules[0], p, NoulAnswer(type="noul", noul=0.99))
...
def choice_rule():
    return Rule.model_validate({
        ...
        "question": {
            ...
            "criteria": {
                "probably_legitimate": "Legitimate",
                "suspicious": "Suspicious",
                "probably_malicious": "Malicious",
                "unknown": "Unknown",
            },
        },
        "policy": {
            "warning_choices": ["suspicious", "probably_malicious"],
...
@pytest.mark.parametrize(
    "choice,p,confidence,status",
    [
        ("suspicious", 0.95, 0.9, "warning"),
        ("probably_malicious", 0.95, 0.1, "uncertain_warning"),
        ("suspicious", 0.4, 0.9, "uncertain_warning"),
        ("unknown", 0.9, 0.9, "unknown"),
        ("probably_legitimate", 0.96, 0.9, "probably_legitimate"),
        ("probably_legitimate", 0.6, 0.4, "unknown"),
    ],
)
def test_choice_policy(snapshot, choice, p, confidence, status):
    ...
    result = judge(
        ...
        ChoiceAnswer(type="choice", choice=choice, confidence=confidence, probabilities=probs),
    )
    assert result.status == status
...
def test_risk_values_are_not_summed(snapshot):
    ...
    answer = ChoiceAnswer(
        ...
        choice="unknown",
        confidence=0.5,
        probabilities={"probably_legitimate": 0.1, "suspicious": 0.4, "probably_malicious": 0.4, "unknown": 0.8},
    )
...
@pytest.mark.parametrize(
    ...
    [
        (2.5, 0.9, "warning"),
        (2.5, 0.2, "uncertain_warning"),
        (1.7, 0.9, "uncertain_warning"),
        (0.5, 0.9, "no_warning"),
        (0.5, 0.1, "unknown"),
    ],
)
def test_custom_score_policy(snapshot, score, conf, status):
    rule = Rule.model_validate({
        ...
        "question": {
            "type": "score",
            "instructions": "Evaluate evidence.",
            "criteria": ["none", "weak", "substantial", "strong"],
        },
    })
    answer = ScoreAnswer(type="score", score=score, confidence=conf, probabilities={str(i): 0.25 for i in range(4)})
